...
export const Schemas = {
    ...
    memberCreate: v.object({ loginName: v.string(80, 3, /^[a-z0-9][a-z0-9._-]*$/), displayName: v.string(100, 1), role: v.enum(['ADMIN', 'EDITOR', 'REVIEWER', 'VIEWER']), extraPermissions: v.array(v.enum(['sensitive.read', 'sensitive.write', 'data.export', 'data.delete']), 4) }),
    memberPermissions: v.object({ expectedRevision: revision, role: v.enum(['ADMIN', 'EDITOR', 'REVIEWER', 'VIEWER']), extraPermissions: v.array(v.enum(['sensitive.read', 'sensitive.write', 'data.export', 'data.delete']), 4) }),
