...
export function visibilityIndex(actor: Actor, clock: Clock, scopes: Scope[], members: ScopeMember[], sources: Source[], blocks: DeletionRequest[] = []) {
    ...
    return { visibleScopeIds, visibleSourceIds, blocked: isBlocked, source(id: string): Source | null { return sourceById.get(id) ?? null; }, scopeVisible,
        ...
        personVisible(person: Person): boolean {
            ...
            return person.workspaceId === actor.workspaceId && !isBlocked('PERSON', person.id) && scopeVisible(person.scopeId)
