...
import { permissionsFor, personVisible, sourceCurrent, scopeVisible } from './policy.ts';
...
export async function handoffCurrent(tx: Tx, h: RecordHandoff, clock: Clock): Promise<boolean> {
    ...
    if (!person || !source || person.sourceId !== source.id || person.status === 'ARCHIVED' || !sourceCurrent(source, clock)) return false;
