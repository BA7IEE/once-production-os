...
for (const route of ROUTES) {
    ...
    const code = ['import.commit', 'job.resume', 'upload.complete'].includes(route.operation) ? '202' : ['person.create', 'source.create', 'scope.create', 'catalog.create', 'import.preview', 'member.create', 'handoff.create', 'upload.create'].includes(route.operation) ? '201' : '200';
