...
需要记录API/DB健康、Worker心跳/积压、上传配额/异常大对象/孤儿量、来源到期/暂停、导出失效、AI未决Attempt/预算、备份成功时间和恢复演练记录。API健康不代表Worker正常；任务失败有工作台与一条可维护告警渠道即可，不为通知先建设统一消息平台。

同一敏感哨兵应在应用/代理/解析日志全检为零。对AI只记录元数据和摘要，不把完整输入prompt写日志。权限读取和维护动作审计可追溯，但不复制整份客户资料到审计表。

## 6. 备份与恢复资产

数据库、物理媒体及其固定locator/checksum清单、必要加密密钥和配置版本都需要可恢复。密钥走独立安全通道，不放普通导出里。导出迁移是成员业务操作，备份是更高权限的运维操作，不能混为一个“下载全库”按钮。
...
3. 使旧Session/激活令牌失效；旧导出保持不可下载；旧AI待发/未决任务进入待核对，不随Worker重启重发。
4. 核对恢复点后的账号停用/角色变化、源用途变更、限制/删除记录。能证明完整则补齐；不能排除缺口的旧数据保持受限，不默认恢复到普通列表。维护人员重新核实后可逐对象恢复内部使用。
