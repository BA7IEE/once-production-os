import { PersonMergePanel } from './merge-ui.tsx';
import { DeletionImpactPanel } from './deletion-ui.tsx';
import { ExportPanel } from './export-ui.tsx';
import { ShortlistWorkbench } from './shortlist-ui.tsx';
import { ProductionPanel, ProductionDetail, PersonProductionPanel } from './production-ui.tsx';
...
function PersonDetail({ id, onClose, onChange }: {
    ...
    const { catalog, me, can, label } = useOS();
    const [production, setProduction] = useState<Selection | null>(null);
    const [refresh, setRefresh] = useState(0);
    ...
    return <><PageTitle overline="BATCH IMPORT" title="预览后，再入库" description="本增量支持最多 100 行结构化 JSON；每一行都绑定同一份明确来源。不自动合并同名人才。" action={<button onClick={() => setRefresh(x => x + 1)}>刷新任务</button>}/><ErrorBox error={a.error ?? sources.error ?? jobs.error}/><section className="panel padded"><form onSubmit={e => {
            ...
            void a.run(async () => {
                ...
                await loadBatch(receipt.resourceId);
                    ...
                    setRefresh(x => x + 1);
                    if (batch?.id === j.aggregateId)
                        void a.run(() => loadBatch(j.aggregateId));
                }}/></td></tr>)}</tbody></table></div> : <Empty title="暂无导入任务"/>}</section>{jobs.data && <Pager page={jobPage} pageSize={20} total={jobs.data.total} setPage={setJobPage}/>}</>;
}
function Members() {
    const { me } = useOS();
    const [refresh, setRefresh] = useState(0);
...
function MemberForm({ member: existing, onClose, onSaved }: {
    ...
    const [extras, setExtras] = useState<Array<'sensitive.read' | 'sensitive.write' | 'data.export' | 'data.delete' | 'data.merge'>>(existing?.extraPermissions ?? []);
    const a = useAction();
    return <Modal title={existing ? '调整成员权限' : '开通内部成员'} onClose={onClose}><form onSubmit={e => {
        ...
        }}><div className="modal-body"><ErrorBox error={a.error}/>{!existing && <><Field label="登录名 *" hint="至少 3 位，小写英文、数字、点、下划线或连字符。"><input required pattern="[a-z0-9][a-z0-9._-]{2,79}" value={name} onChange={e => setName(e.target.value)}/></Field><Field label="显示名称 *"><input required maxLength={100} value={display} onChange={e => setDisplay(e.target.value)}/></Field></>}<Field label="基础角色"><select value={role} onChange={e => setRole(e.target.value as Me['role'])}><option value="VIEWER">只读成员</option><option value="EDITOR">资料维护</option><option value="REVIEWER">资料核验</option><option value="ADMIN">管理员</option></select></Field><Field label="受限字段附加权限"><div className="check-grid">{(['sensitive.read', 'sensitive.write', 'data.export', 'data.delete', 'data.merge'] as const).map(p => <label className="check-chip" key={p}><input type="checkbox" checked={extras.includes(p)} onChange={e => setExtras(e.target.checked ? [...extras, p] : extras.filter(x => x !== p))}/>{p === 'sensitive.read' ? '读取联系方式 / 原文' : p === 'sensitive.write' ? '维护联系方式 / 原文' : p === 'data.export' ? '内部 JSON 导出' : p === 'data.delete' ? '删除影响评估 / 清理' : '受控人才合并'}</label>)}</div></Field><p className="muted">附加权限不会绕过资料的访问范围和有效期限。新成员通过一次性凭证自行设置密码，没有默认口令。</p></div><footer className="modal-footer"><button type="button" onClick={onClose}>取消</button><Submit busy={a.busy}>{existing ? '保存权限' : '创建并显示一次性凭证'}</Submit></footer></form></Modal>;
}
...
function Catalog() {
    ...
    const [creating, setCreating] = useState(false);
    return <><PageTitle overline="CONTROLLED VOCABULARY" title="分类字典" description="角色、城市、语言、技能、行业和作品类型统一使用稳定代码。停用只影响新引用，不破坏已有事实。" action={<button className="primary" onClick={() => setCreating(true)}>＋ 新增分类项</button>}/><div className="tabs">{([['role', '制作角色'], ['city', '常驻城市'], ['language', '工作语言'], ['skill', '擅长类型'], ['industry', '行业'], ['workType', '作品类型']] as const).map(([code, name]) => <button key={code} className={ns === code ? 'active' : ''} onClick={() => setNs(code)}>{name}</button>)}</div><section className="panel"><div className="table-wrap"><table><thead><tr><th>中文名称</th><th>英文名称</th><th>稳定代码</th><th>状态</th><th /></tr></thead><tbody>{catalog.filter(c => c.namespace === ns).map(c => <tr key={c.id}><td>{c.labelZh}</td><td>{c.labelEn || '—'}</td><td><code>{c.code}</code></td><td>{c.status === 'ACTIVE' ? '启用' : '停用'}</td><td><button onClick={() => setEdit(c)}>编辑</button></td></tr>)}</tbody></table></div></section>{(creating || edit) && <CatalogForm item={edit ?? undefined} namespace={ns} onClose={() => { setCreating(false); setEdit(null); }} onSaved={() => { setCreating(false); setEdit(null); refreshCatalog(); }}/>}</>;
}
