...
function RootForm({ kind, existing, catalog, onClose, onDone }: {
    ...
    const submit = () => {
        ...
        if (kind === 'work')
            void m.submit(existing ? 'work.update' : 'work.create', existing ? { expectedRevision: existing.revision, title, description, industryCode: industryCode || null, workTypeCodes, origin, originNote } : { title, description, industryCode: industryCode || null, workTypeCodes, origin, originNote, ...sourceFields }, existing ? { id: existing.id } : {});
        else
