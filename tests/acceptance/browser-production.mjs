...
try {
 ...
 await until(async()=>['COMPLETED','RETAINED_WITH_BASIS'].includes((await prisma.deletionRequest.findUnique({where:{id:blockRequestId}}))?.state??''));
 await blockDetail.getByText('删除流程已完成',{exact:true}).waitFor();
 await blockDetail.getByText('最终处置证据',{exact:true}).waitFor();
 const finalizedDeletion=await prisma.deletionRequest.findUniqueOrThrow({where:{id:blockRequestId}});
 assert.equal(finalizedDeletion.state,'COMPLETED');
 assert.equal(finalizedDeletion.executionPlanDigest?.length,64);
 assert.ok(finalizedDeletion.dependencyCleanupCompletedAt);
 assert.ok(finalizedDeletion.rootFinalizedAt);
 assert.equal(finalizedDeletion.rootFinalizationEvidenceDigest?.length,64);
 assert.equal(finalizedDeletion.cleanupErrorCode,null);
 const erasedProject=await prisma.project.findUniqueOrThrow({where:{id:projectId}});
 assert.equal(erasedProject.status,'ERASED');
 assert.equal(erasedProject.title,'[ERASED]');
 assert.equal(erasedProject.brief,'');
 assert.equal(erasedProject.locationNote,'');
 assert.equal(erasedProject.dateNote,'');
 assert.equal(erasedProject.reviewNote,'');
 ...
 assert.equal(await getStatus(owner,'/projects/'+projectId),404);
 assert.ok(!(await json(owner,'/projects')).items.some(x=>x.id===projectId));
 console.log('PASS DEV-07F browser: CLEANING -> COMPLETED; Project becomes ERASED minimal header and remains unusable');
